...
class Run(AbstractContextManager):
    ...
    def log_histograms(
        ...
        Args:
            histograms (dict[str, Histogram]): A dictionary with attribute names as keys
                and histogram objects as values.
            step (float or int): Index of the log entry.
                Tip: Using float rather than int values can be useful, for example, when logging substeps in a batch.
            timestamp (datetime, optional): Time of logging the histogram. If not provided, the current time is used.
                If provided, and `timestamp.tzinfo` is not set, the time is assumed to be in the local timezone.
        ...
        Examples:
            ...
            run.log_histograms(
                histograms={
                    "layers/1/activation": my_histogram1,
                    "layers/2/activation": my_histogram2,
                },
                step=1,
            )
            ```
